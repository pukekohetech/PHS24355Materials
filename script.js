...
/* --------------------------------------------------------------
   Share helper – PDF + text body, fallback to mailto
   -------------------------------------------------------------- */
async function sharePDF(file) {
  if (!finalData) return;
  ...
  const bodyText = buildEmailBody(finalData);
  ...
  // Try Web Share with file + body text
  ...
  // And open an email with the report in the body
  const mailto =
...
async function emailWork() {
  ...
  const stampedFile = new File([pdfBlob], {
