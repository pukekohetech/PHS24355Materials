...
// Simple XOR obfuscation for answer security
const XOR_KEY = 42;
const xor = s =>
  btoa(
    [...s]
...
// Global app metadata loaded from JSON
let APP_TITLE, APP_SUBTITLE, TEACHERS, ASSESSMENTS;

/* --------------------------------------------------------------
   Load questions.json – populates global config and assessments
   -------------------------------------------------------------- */
async function loadQuestions() {
  try {
    ...
    // Convert regex strings to RegExp objects for rubric matching
...
function initApp() {
  ...
  const usernameEl = document.getElementById("username"); // New field
  ...
  teacherSel.innerHTML = '<option value="">Select Teacher</option>'; // Reset
  TEACHERS.forEach(t => {
...
/* --------------------------------------------------------------
   Save student info (name, ID, teacher, username)
   -------------------------------------------------------------- */
...
/* --------------------------------------------------------------
   Load selected assessment and render questions
   -------------------------------------------------------------- */
...
/* --------------------------------------------------------------
   Save individual answer with XOR encryption
   -------------------------------------------------------------- */
...
/* --------------------------------------------------------------
   Grade all questions using rubric
   -------------------------------------------------------------- */
function gradeIt() {
  let total = 0;
  const results = [];
...
function submitWork() {
  ...
  const totalPoints =
    ...
    currentAssessment.questions.reduce((sum, q) => sum + (q.maxPoints || 0), 0);
  ...
  // Update result UI
  document.getElementById("student").textContent = `${name} (${username})`;
...
function buildEmailBody(fd) {
  ...
  lines.push(`Student: ${fd.name} (${fd.username}) – ID: ${fd.id}`);
...
/* --------------------------------------------------------------
   Share PDF (Web Share API or fallback)
   -------------------------------------------------------------- */
async function sharePDF(file) {
  ...
  const shortBody = [
    ...
    `Student: ${finalData.name} (${finalData.username}) – ID: ${finalData.id}`,
...
/* --------------------------------------------------------------
   Generate PDF (A4 Portrait, with username)
   -------------------------------------------------------------- */
async function emailWork() {
  ...
  const addHeader = () => {
    ...
    try {
      ...
      img.onload = () => pdf.addImage(img, "PNG", pageWidth - margin - 28, 5, 28, 28);
    } catch (_) {}
  };

  addHeader();

  // Student info with username
  pdf.setTextColor(0, 0, 0);
  pdf.setFontSize(11);
  pdf.text(
    `${finalData.name} (${finalData.username}) • ID: ${finalData.id} • ${finalData.teacherName} • ${finalData.submittedAt}`,
    margin,
    40
  );
  ...
  pdf.rect(margin, 45, 60, 15, "F");
  pdf.setTextColor(26, 73, 113);
  pdf.setFontSize(16);
  pdf.setFont("helvetica", "bold");
  pdf.text(`${finalData.points}/${finalData.totalPoints} (${finalData.pct}%)`, margin + 4, 55);

  // Add content (single or multi-page)
  let yPosition = 70;
...
function createToastElement() {
  ...
  t.style.cssText = `
    ...
    font-family: inherit; font-size: 0.95rem; min-width: 200px; text-align: center;
  `;
...
/* --------------------------------------------------------------
   Anti-cheating: block paste, copy, cut, context menu
   -------------------------------------------------------------- */
