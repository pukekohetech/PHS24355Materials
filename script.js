...
function buildEmailBody(fd) {
  // Prefer the stored username, but fall back to the auto-detected device login
  ...
  // Student line – now always shows the login ID if we have it
  lines.push(
    `Student: ${fd.name}${
      username
        ? ` (${username}${detectedUsername && detectedUsername === username ? " (Device Auto)" : ""})`
        : ""
    } – ID: ${fd.id}`
  );

  // Teacher line – include email only if it's actually set
  lines.push(
